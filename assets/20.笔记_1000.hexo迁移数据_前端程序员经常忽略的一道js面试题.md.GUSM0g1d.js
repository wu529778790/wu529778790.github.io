...
</code></pre><h2 id="第二问" tabindex="-1">第二问 <a class="header-anchor" href="#第二问" aria-label="Permalink to &quot;第二问&quot;">​</a></h2><pre><code>直接调用getName函数，就是访问当前尚文作用域内的叫getName的函数，所以这里应该直接把关注点放在4和5上，跟123都没什么关系。此处有两个坑，一是变量生命提升，而是函数表达式和函数声明的区别。
...
</code></pre><h2 id="第三问" tabindex="-1">第三问 <a class="header-anchor" href="#第三问" aria-label="Permalink to &quot;第三问&quot;">​</a></h2><pre><code>Foo().getName();先执行了Foo函数，然后再调用Foo函数的返回值对象的getName属性函数
...
之后Foo函数的返回值是this，简单的讲，this的指向室友所在函数的调用方式决定的，而此处的直接调用方式，this指向window对象。所以Foo函数返回的是window对象，相当于执行window.getname(),而window种的getName已经被修改为alert(1),所以最终会输出1
    ...
    window.Foo().getName()
</code></pre><h2 id="第四问" tabindex="-1">第四问 <a class="header-anchor" href="#第四问" aria-label="Permalink to &quot;第四问&quot;">​</a></h2><pre><code>直接调用getName函数，相当于window.getName(),因为这个变量已经被Foo函数执行时修改了，所以结果与第三问相同，为1，也就是说Foo执行后把全局的getName函数给重写了一次，所以结果就是Foo()执行重写的拿个getName函数。
</code></pre><h2 id="第五问" tabindex="-1">第五问 <a class="header-anchor" href="#第五问" aria-label="Permalink to &quot;第五问&quot;">​</a></h2><pre><code>第五问的 new Foo.getName() 此处考查的是js的运算符优先级问题
...
</code></pre><h2 id="第六问" tabindex="-1">第六问 <a class="header-anchor" href="#第六问" aria-label="Permalink to &quot;第六问&quot;">​</a></h2><pre><code>这一题比上一题的区别就在Foo那里多了一个括号，这个有括号跟没括号在我们上疑问中就看出来优先级是有区别的
...
</code></pre><h2 id="第七问" tabindex="-1">第七问 <a class="header-anchor" href="#第七问" aria-label="Permalink to &quot;第七问&quot;">​</a></h2><pre><code>new new Foo().getName(); 同样是运算符优先级问题
...
</code></pre>`,16),e("p",{alert:"","(5);":""},"##答案 function Foo() { getName = function () { alert (1); }; return this; } Foo.getName = function () { alert (2);}; Foo.prototype.getName = function () { alert (3);}; var getName = function () { alert (4);}; function getName()",-1),e("pre",null,[e("code",null,`    //答案：
    Foo.getName();//2
    getName();//4
    Foo().getName();//1
    getName();//1
