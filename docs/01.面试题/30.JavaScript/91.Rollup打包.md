...
这时，我们再在`example/index.html`中引入上面打包生成的`bundle.js`文件，打开浏览器： ![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/8596f50fa8ad44e48fccad3de36a9cbb~tplv-k3u1fbpfcp-zoom-1.image) 如我们所预料的，控制台输出了`柯森`。
...
会得到以下报错： ![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/f5730d429a8a4d61ab28a839f3343394~tplv-k3u1fbpfcp-zoom-1.image) 打包后的`bundle.js`仍然会在`Node.js`中工作，但是`the-answer`不包含在包中。为了解决这个问题，将我们编写的源码与依赖的第三方库进行合并，`rollup.js`为我们提供了`resolve`插件。
...
这时再次执行`npm run build`，可以发现报错已经没有了： ![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/4712a288683f44f2a871b2c518b02ab5~tplv-k3u1fbpfcp-zoom-1.image)
...
然后打包，发现会出现报错： ![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/c664d97500e84fba8a6c3e5e23b589fd~tplv-k3u1fbpfcp-zoom-1.image) 提示我们缺少`@babel/core`，因为`@babel/core`是`babel`的核心。我们来进行安装：
...
执行打包，发现会发生如下报错： ![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/20c6782fd3d34c3b93c1178fbfa69338~tplv-k3u1fbpfcp-zoom-1.image) 提示我们缺少`@rollup/plugin-json`插件来支持`json`文件。
